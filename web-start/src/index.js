...
function loadMessages() {
  ...
  onSnapshot(recentMessages, function (snapshot) {
...
async function saveImageMessage(file) {
  ...
  try {
    ...
    console.log('There was an error uploading image to Cloud Storage', error);
  }
}
...
async function saveMessagingDeviceToken() {
  ...
  try {
    ...
    if (currentToken) {
      // Save the device token to the Cloud Firestore
      const tokenRef = doc(getFirestore(), 'fcmTokens', currentToken);
      await setDoc(tokenRef, {
        uid: getAuth().currentUser.uid
      });

      // This will fire when a message is received when the app is in the foreground
      // When the app is in the background, firebase-messaging-sw will receive the message instead
